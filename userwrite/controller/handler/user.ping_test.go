...
func TestPing(t *testing.T) {
	...
	tests.TestNewRequest(t, "/v1/user/ping", "", "POST", Ping, nil, "application/json", map[string]string{}, 200, true)
}
...
func TestPing2(t *testing.T) {
	...
	bodyResponse := `{"pong":"🏓"}`
	app := fiber.New()
	app.Post("/v1/user/ping", Ping)

	req := httptest.NewRequest("POST", "/v1/user/ping", nil)
